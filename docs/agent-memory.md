...
- Read-only live-provider routes use a configurable short server-side cache with request coalescing and freshness metadata; inactive tab status panels lazy-load on first activation so initial refreshes do not duplicate provider or planning calls.
